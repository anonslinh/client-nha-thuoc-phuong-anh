...
<div class="container-fluid">
    <div class="card card-body py-3">
        <div class="d-sm-flex align-items-center justify-space-between">
            <div>
                <h4 class="mb-1 card-title">Preview bệnh</h4>
                <div class="text-muted">
                    Danh mục: {{ $category->name }} |
                    {{ $category->type == 1 ? 'Bệnh theo mùa' : 'Bệnh theo đối tượng' }}
                </div>
            </div>
            <a href="{{ route('catalog_v1.diseases.items', $category->id) }}" class="btn btn-danger">
                Quay lại
            </a>
        </div>
    </div>
    ...
    @php
        $avatar = $item->avatar ? (\Illuminate\Support\Str::startsWith($item->avatar,'http') ? $item->avatar : asset($item->avatar)) : null;
            ...
            <div class="d-flex align-items-center gap-3 mb-4">
                ...
                <div>
                    ...
                    <div class="small text-muted">
                        Ngày đăng:
                        {{ $item->posted_at ? \Carbon\Carbon::parse($item->posted_at)->format('d/m/Y H:i') : '-' }}
                        |
                        Trạng thái:
