...
<div class="container-fluid">
    <div class="card card-body py-3">
        <div class="d-sm-flex align-items-center justify-space-between">
            <h4 class="mb-0 card-title">Hạng mục bệnh</h4>
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('catalog_v1.diseases.categories') }}" method="get" class="row">
                ...
                <div class="col-md-3 mb-2">
                    <select name="type" class="form-control">
                        ...
                        <option value="1" {{ request('type') == '1' ? 'selected' : '' }}>Bệnh theo mùa</option>
                        <option value="2" {{ request('type') == '2' ? 'selected' : '' }}>Bệnh theo đối tượng</option>
                    </select>
                </div>
                <div class="col-md-5 mb-2">
                    <button class="btn btn-primary" style="margin-right:10px">Tìm kiếm</button>
                    <a href="{{ route('catalog_v1.diseases.categories') }}" class="btn btn-danger">Hủy</a>
                </div>
            </form>
                ...
                @forelse($listData as $row)
                        ...
                        <div class="card h-100 shadow-sm border-0" style="border-radius:18px; overflow:hidden;">
                            ...
                            <div class="card-body">
                                <div class="d-flex align-items-center gap-3 mb-3">
                                    ...
                                    <div>
                                        ...
                                        <div class="small text-muted">
                                            {{ $row->type == 1 ? 'Bệnh theo mùa' : 'Bệnh theo đối tượng' }}
                                        </div>
                                    </div>
                                </div>

                                <p class="text-muted mb-3" style="min-height:48px;">{{ $row->short_description }}</p>

                                <div class="mb-3 d-flex gap-2 flex-wrap">
                                    <span class="badge bg-primary">Bệnh: {{ $count }}</span>
                                ...
                                <div class="d-flex flex-wrap gap-2">
                                    <a href="{{ route('catalog_v1.diseases.items',$row->id) }}" class="btn btn-primary btn-sm">Danh sách bệnh</a>
                                    <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#modalUpdate{{$row->id}}">Sửa</button>
                                    <a href="{{ route('catalog_v1.diseases.categories.destroy',$row->id) }}" class="btn btn-danger btn-sm btn-sa-confirm">Xóa</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="modal fade" id="modalUpdate{{$row->id}}" tabindex="-1">
                        <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
                            <form action="{{ route('catalog_v1.diseases.categories.update',$row->id) }}" method="post" class="modal-content" enctype="multipart/form-data">
                                @csrf
                                <div class="modal-header d-flex align-items-center">
                                    <h4 class="modal-title">Cập nhật hạng mục bệnh</h4>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                </div>
                                <div class="modal-body">
                                    <div class="row">
                                        <div class="col-md-5 mb-2">
                                            <label class="form-label">Tên bệnh / hạng mục</label>
                                            <input class="form-control" name="name" value="{{ $row->name }}" required>
                                        </div>
                                        <div class="col-md-3 mb-2">
                                            <label class="form-label">Type</label>
                                            <select class="form-control" name="type" required>
                                                <option value="1" {{ $row->type==1?'selected':'' }}>Bệnh theo mùa</option>
                                                <option value="2" {{ $row->type==2?'selected':'' }}>Bệnh theo đối tượng</option>
                                            </select>
                                        </div>
...
<div class="modal fade" id="modalCreate" tabindex="-1">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <form action="{{ route('catalog_v1.diseases.categories.store') }}" method="post" class="modal-content" enctype="multipart/form-data">
            @csrf
            <div class="modal-header d-flex align-items-center">
                <h4 class="modal-title">Thêm hạng mục bệnh</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-3 mb-2">
                        <label class="form-label">Type</label>
                        <select class="form-control" name="type" required>
                            <option value="1" selected>Bệnh theo mùa</option>
                            <option value="2">Bệnh theo đối tượng</option>
                        </select>
                    </div>
                    <div class="col-md-2 mb-2">
                        <label class="form-label">Sort</label>
