...
<div class="container-fluid">
    <div class="card card-body py-3">
        <div class="d-sm-flex align-items-center justify-space-between">
            <div>
                <h4 class="mb-1 card-title">Bài viết - {{ $category->name }}</h4>
                <div class="text-muted">Quản trị nội dung Góc sức khỏe theo danh mục</div>
            </div>
            <div class="d-flex gap-2">
                <button class="btn btn-info" data-bs-toggle="modal" data-bs-target="#modalCreate">
                    <i class="ti ti-plus me-1"></i> Thêm bài viết
                </button>
                <a href="{{ route('catalog_v1.health_corner.categories') }}" class="btn btn-danger">Quay lại</a>
            </div>
        </div>
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('catalog_v1.health_corner.articles',$category->id) }}" method="get" class="row">
                ...
                <div class="col-md-4 mb-2">
                    ...
                    <a href="{{ route('catalog_v1.health_corner.articles',$category->id) }}" class="btn btn-danger">Hủy</a>
                </div>
            </form>
                ...
                @forelse($listData as $row)
                            ...
                            <div class="card-body">
                                <div class="d-flex gap-3 mb-3">
                                    ...
                                    <div class="flex-grow-1">
                                        ...
                                        <div class="small text-muted">Ngày đăng: {{ $row->posted_at ? \Carbon\Carbon::parse($row->posted_at)->format('d/m/Y H:i') : '-' }}</div>
                                    </div>
                                </div>
                                ...
                                <div class="d-flex flex-wrap gap-2">
                                    <a href="{{ route('catalog_v1.health_corner.articles.show',[$category->id,$row->id]) }}"
                                    target="_blank"
                                    ...
                                    <a href="{{ route('catalog_v1.health_corner.articles.destroy',[$category->id,$row->id]) }}"
                    ...
                    <div class="modal fade" id="modalUpdate{{$row->id}}" tabindex="-1">
                        <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
                            <form action="{{ route('catalog_v1.health_corner.articles.update',[$category->id,$row->id]) }}" method="post" class="modal-content" enctype="multipart/form-data">
                                @csrf
...
<div class="modal fade" id="modalCreate" tabindex="-1">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <form action="{{ route('catalog_v1.health_corner.articles.store',$category->id) }}" method="post" class="modal-content" enctype="multipart/form-data">
            @csrf
            <div class="modal-header d-flex align-items-center">
