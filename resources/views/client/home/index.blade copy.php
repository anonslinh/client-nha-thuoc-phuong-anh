                    ...
                    <div class="col-lg-2">
                        <div class="logo">
                            <a href="{{ url('/') }}">
                                <img style="
    width: 120px;
" src="https://1986hotelscualo.com/assets/assetsclient/img/logo_version2.png" alt="">
                            </a>
                        </div>
                    </div>
                    <div class="col-lg-10">
    ...
    <div class="modal fade" id="availabilityModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
            ...
            <div class="modal-footer d-flex justify-content-between">
                ...
                <small>
                    Mỗi ô: <b>Đã đặt / Còn trống / Tổng</b>. Ô xanh = đủ phòng cho khoảng ngày đang chọn.
                </small>
                </div>
            ...
            <div class="row">
                ...
                <div class="booking-form">
                    ...
                    <div class="check-date">
                        ...
                        <input type="text" class="date-input" id="date-in" placeholder="Chọn ngày nhận">
                        <i class="icon_calendar"></i>
                    </div>

                    <div class="check-date">
                        <label for="date-out">Ngày trả:</label>
                        <input type="text" class="date-input" id="date-out" placeholder="Chọn ngày trả">
                        <i class="icon_calendar"></i>
                    </div>
    ...
    <script>
(() => {
  const apiUrl = @json(route('client.rooms.availability'));
  ...
  let state = {
    ...
    roomId: null,
    qty: 1,
    month: null,          // YYYY-MM (month đang hiển thị trong popup)
    data: null,           // json từ api
    picking: { start: null, end: null }, // click chọn range trên lịch
  };
  ...
  // flatpickr
  flatpickr(elIn, { dateFormat: "Y-m-d" });
  flatpickr(elOut, { dateFormat: "Y-m-d" });
  ...
  async function fetchMonth(month) {
    const res = await fetch(`${apiUrl}?month=${encodeURIComponent(month)}`, { headers: { 'Accept':'application/json' }});
    if (!res.ok) throw new Error('Không tải được dữ liệu lịch');
    return await res.json();
  }

  function inRange(date, start, end) {
    // [start, end) (end là ngày trả)
    return date >= start && date < end;
  }

  function renderCalendarTable() {
    const data = state.data;
    const days = data.days; // list YYYY-MM-DD trong tháng
        ...
        <td>
          <div class="d-flex justify-content-between align-items-center">
            <div>
              ...
              <small class="text-muted">Mặc định: ${r.default_total} phòng</small>
            </div>
          </div>
        </td>
      `;
    ...
    // bind click chọn ngày trực tiếp trên lịch (2 click)
  ...
  function updateBookButtonState() {
    ...
    // điều kiện enable nút: có checkin/checkout hợp lệ + TẤT CẢ ngày trong range đủ remain
    let okAll = true;
    let badDays = [];

    if (!state.checkIn || !state.checkOut) okAll = false;
    if (state.checkIn && state.checkOut && state.checkOut <= state.checkIn) okAll = false;

    if (okAll && selectedRoom) {
      // duyệt từng ngày: [checkIn, checkOut)
      let d = state.checkIn;
      while (d < state.checkOut) {
        const cell = selectedRoom.days[d];
        const remain = Number(cell.remain || 0);
        const status = cell.status || 'open';
  ...
  function onClickDay(dateStr) {
    // click 2 lần để tạo range: start -> end (end = start+1 nếu click 1 ngày)
    if (!state.picking.start || (state.picking.start && state.picking.end)) {
      ...
      // checkOut phải là ngày sau end => chọn end inclusive trên UI
      state.checkIn = start;
    ...
    // sync lại input form (để người dùng thấy)
  ...
  btnPrevMonth.addEventListener('click', async () => {
    const d = new Date(state.data.start);
    d.setMonth(d.getMonth() - 1);
    await openModalAndLoad(ym(d));
  });
  ...
  btnBookNow.addEventListener('click', () => {
    // Redirect sang trang danh sách phòng /phong với query
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const checkIn = elIn.value.trim();
    const checkOut = elOut.value.trim();
    const roomId = elRoom.value;
    const qty = Number(elQty.value || 1);

    if (!checkIn || !checkOut) return showErr('Vui lòng chọn ngày nhận và ngày trả.');
    ...
    const month = checkIn.slice(0,7); // YYYY-MM theo tháng của ngày nhận

    try {
      await openModalAndLoad(month);
    } catch (err) {
      showErr(err.message || 'Có lỗi xảy ra.');
    }
  });
